...
void teacher_info()
{
    ...
    while(flag)
    {
        ...
        switch(choose)
        {
            ...
            case 2:s=change_password(s);break;
...
void search_data(struct teacher*tea_hand,struct student*stu_hand)
{
    ...
    while(x)
    {
        struct teacher *new_s;
        new_s = tea_hand;
        struct student *new_ps;
        new_ps = stu_hand;
        printf("-------------------------------------\n");
...
void teacher_write(struct teacher*current)
{
     ...
     while(fgets(line,sizeof(line),fp)!=NULL)
     {
	     ...
	     {
		     found = 1;
		     strcpy(temp->password,current->password);
		     fseek(fp,-strlen(line),SEEK_CUR);
		     fprintf(fp,"%s %s %s %d\n",temp->name,temp->password,temp->id,temp->age);
		     break;
	     }
     }
	if(found!=1)
...
void stu_write(struct student*current)
{
     ...
     while(fgets(line,sizeof(line),fp)!=NULL)
     {
	     ...
	     if(strcmp(temp->id,id_found)==0)
	     {
		     ...
		     strcpy(temp->password,current->password);
		     fseek(fp,-strlen(line),SEEK_CUR);
		     fprintf(fp,"%s %s %s %d %s\n",temp->name,temp->password,temp->id,temp->age,temp->class);
		     break;
	     }
     }
	if(found!=1)
